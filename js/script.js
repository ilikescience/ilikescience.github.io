...
const drawTenPrint = canvas => {
  ...
  const columns = 50;
  const colWidth = (canvas.offsetWidth) / columns;
  const rows = Math.ceil(canvas.offsetHeight / colWidth);
  
  drawLine = (i, j) => {
    const random = Math.round(Math.random())
    const color = `hsl(0, 0%, ${Math.round(Math.random() * 100)}%)`;
    ctx.strokeStyle = color;
    ctx.beginPath();
    switch (random) {
      case 0:
      ctx.moveTo(0, 0);
      ctx.lineTo(colWidth, colWidth);
      break;
      case 1:
      ctx.moveTo(0, colWidth);
      ctx.lineTo(colWidth, 0);
      break;
      default:
      // do nothing
    }
  ...
  for (let i = 0; i < rows; i ++) {
    for (let j=0; j < columns; j++) {
      ctx.save();
      ctx.translate(j * colWidth, i * colWidth);
      drawLine(i, j);
      ctx.restore();
    }
  }
